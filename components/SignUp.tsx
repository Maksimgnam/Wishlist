...
const SignUp:FC<Auth> = ({isChange}) => {
  ...
  return (
    <div className='w-80 h-96 border border-pink-500  shadow-2xl  rounded-xl flex flex-col  items-center  justify-between p-6'>
        <h2 className='text-2xl text-black font-medium'> Sign up</h2>
        <div className='w-full h-44 flex flex-col justify-between' >
            <input type="text"  value={name} onChange={(e) => setName(e.target.value)} className='w-full h-12 bg-white text-black border border-custom rounded outline-none pl-2' placeholder='Name' />
        ...
        <button  onClick={signUp} className='w-full h-12  rounded bg-button'>
          <p className='text-lg text-white font-medium'>Sign up</p>
        </button>
  
        <div className='w-full h-6 flex items-center  justify-end' >
            <div className='w-auto h-auto flex '>
                <p className='text-sm font-medium text-pink-500 cursor-pointer mr-3'>Sign up</p>
